...
const FollowerModal = () => {
  ...
  return (
      <div>
          <div className="followedlength">
              {followedMembers.length}
              </div>
      <button onClick={changedcontent} className="followingbutton">Following </button>
      {popupFollwing && (
              ...
              {followedMembers.map((pop) => {
                return (
                  <div className="pop_up_card">
                        <div key={pop._id}>
                        <div>    
                      <Link className="name" to={`/UserProfile/${pop._id}`}>{pop.name}</Link>
</div>
       ...
       <div className="followerslength">
              {followers.length}
              </div>
      <button className="followerbutton"  onClick={changedFollower}>Followers</button>
      {popupFollower && (
              ...
              {followers.map((pop) => {
                return (
                  <div className="pop_up_card">
                    ...
                    <img className="w-12 min-w object-cover rounded-full shadow-lg  followersprofile"src={pop.imageURL}></img>
